...
const ApiLogo = () => (
    <svg width="99px" height="25px" viewBox="0 0 99 25" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" className="ms-3">
        <g id="HiFi" stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
            <g id="Desktop-HD-Copy" transform="translate(-1017.000000, -2562.000000)" fill="#D51007" fillRule="nonzero">
...
const RecentTracks = () => {
    ...
    return (
        <React.Fragment>
            <Section className="d-flex flex-column pt-3" background="white">
                <MaxWidthContainer>
                    ...
                    <MobileView className="row" isToggled={ isToggled }>
                        {recentTracks.map((rt, index) => (
                            <div className={`col-8 col-sm-4 col-md-3 col-xl-3 col-xxl-2 ${isToggled && 'mb-md-5'}`} key={ index }>
                                <a href={rt.url} style={{ textDecoration: 'none'}} target="_blank" title={`${rt.name} - ${rt.artist.name} - ${rt.album['#text']}`}>
